...
import { cloudUploadOutline, documentTextOutline, trashOutline, cloudDownloadOutline, documentAttachOutline } from 'ionicons/icons'; // Use documentAttachOutline for convert
...
const WordToPdf = () => {
  ...
  const acceptedFiles = ".doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document";
  ...
  const handleDrop = (event) => {
    ...
    if (droppedFiles && droppedFiles.length > 0) {
      ...
      const newFiles = Array.from(droppedFiles).filter(file =>
           validWordTypes.includes(file.type) || validWordExtensions.some(ext => file.name.toLowerCase().endsWith(ext))
      );
      if (newFiles.length !== droppedFiles.length) {
        presentToast({ message: 'Some non-Word files (.doc, .docx) were ignored.', duration: 3000, color: 'warning' });
      }
  ...
  const handleSelectFileClick = () => { /* ... (same) ... */
  ...
  const handleConvertSubmit = async () => {
    ...
    if (selectedFiles.length < 1) {
      presentToast({ message: 'Please select at least one Word file to convert.', duration: 3000, color: 'warning' });
      return;
    }
    // --- END CHANGE ---
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Keep specific error hints
         if (response.status === 500 && errorMsg.toLowerCase().includes('libreoffice') || errorMsg.toLowerCase().includes('conversion tool')) {
             errorMsg = "Conversion failed. Ensure LibreOffice (Linux/Mac) or MS Office (Windows) is installed correctly on the server.";
        } else if (response.status === 500 && errorMsg.toLowerCase().includes('docx2pdf')) {
             errorMsg = "Conversion failed. Ensure docx2pdf requirements (like MS Office COM) are met on the Windows server.";
        }
      ...
      presentToast({ message: 'Word documents converted to PDFs successfully! Check downloads for the ZIP file.', duration: 4000, color: 'success' });
      // --- END CHANGE ---
      setSelectedFiles([]); // Clear list on success

    } catch (error) {
      console.error('Error converting Word to PDF:', error);
      presentToast({ message: `Conversion Error: ${error?.message || 'Please try again.'}`, duration: 5000, color: 'danger' });
    } finally {
      setIsLoading(false);
    }
  };
               ...
               >
                {isLoading ? <IonSpinner name="crescent" /> : (
                   <>
                     ...
                     Convert {selectedFiles.length} Word File{selectedFiles.length !== 1 ? 's' : ''} to PDF
                   </>
                 )}
              </IonButton>
